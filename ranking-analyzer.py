    ...
    def check_satd(self):
        ...
        for commit in self.repo.iter_commits('master'):
            if not pre is None:
                diff = [
                    (test.diff) for test
                    in commit.diff(pre, create_patch=True)
                ]
                for line in self.DELIMITER.join(diff).splitlines():
                    if any([word.lower() in line.lower() for word in self.debt_words]) and line.startswith("-"):
                        if pre.committer in alluser:
                            alluser[pre.committer] = int(alluser[pre.committer]) + 1
                        else:
                            alluser[pre.committer] = 1
            pre = commit
